...
// FIX: state.stats does not exist in state — replaced with state.history for streak/sessions
export function renderRoadmap(state) {
  ...
  // FIX: completedSessions not in history — using entries.filter(done).length as approximation
  const completedSessions = (history.entries || []).filter((e) => e.outcome === 'done').length;
  setText('rm-sess', completedSessions);
  setText('rm-streak', `${history.successStreak || 0}🔥`);
