...
let CLOUD_KV = {};
const ADMIN_EMAILS=['ramaman747@gmail.com'];
...
/* ── Error log ── */
const _errorLog=[];
let _cloudSyncFailCount=0;
function logAppError(context,err,detail=''){
  const entry={ts:new Date().toLocaleTimeString(),context:String(context||'app'),message:String(err?.message||err||'unknown error'),detail:String(detail||err?.code||'')};
  _errorLog.push(entry);
  if(_errorLog.length>50) _errorLog.shift();
  console.warn('[StriveAI]',entry.context,':',entry.message,entry.detail?'('+entry.detail+')':'');
}

function mapAIErrorMessage(error,action='chat'){
  const code=String(error?.code||'').toLowerCase();
  const msg=String(error?.message||'');
  if(code==='config'||code==='config_error') return 'AI is not configured. Contact the administrator to set the API key.';
  if(error instanceof TypeError||code==='network_error'||msg.toLowerCase().includes('failed to fetch')) return 'Cannot reach the AI server. Check your internet connection and try again.';
  ...
  if(code==='timeout') return action==='roadmap'
    ? 'Roadmap generation timed out. Your connection may be slow — please try again.'
  ...
  if(code==='rate_limited'||code==='429'||msg.includes('429')) return 'Too many AI requests. Please wait a moment before trying again.';
...
async function aiRequest(prompt, systemCtx='', maxTokens=1000, opts={}, action='chat') {
  ...
  // Track AI usage for billing/usage counters
  S.progress.aiCallCount=(S.progress.aiCallCount||0)+1;
  return {
    text:data.text||'',
...
function parseTaskDetailContract(raw){
  ...
  for(const line of body){
    ...
    // New rich fields
    match=line.match(/^DIFFICULTY\s*\|\s*(easy|medium|hard)$/i);
    if(match){
      if(!seen.has('difficulty')){seen.add('difficulty');parsed.difficulty=match[1].toLowerCase();}
      continue;
    }
    match=line.match(/^TIME_MINUTES\s*\|\s*(\d+)$/i);
    if(match){
      if(!seen.has('time_minutes')){seen.add('time_minutes');parsed.time_minutes=parseInt(match[1],10);}
      continue;
    }
    match=line.match(/^SUCCESS_CRITERIA\s*\|(.+)$/i);
    if(match){
      if(!seen.has('success_criteria')){seen.add('success_criteria');parsed.success_criteria=match[1].split('|').map(s=>s.trim()).filter(Boolean);}
      continue;
    }
    match=line.match(/^WHAT_NOT_TO_DO\s*\|(.+)$/i);
    if(match){
      if(!seen.has('what_not_to_do')){seen.add('what_not_to_do');parsed.what_not_to_do=match[1].split('|').map(s=>s.trim()).filter(Boolean);}
      continue;
    }
    match=line.match(/^WEAKEST_ASSUMPTION\s*\|\s*(.+)$/i);
    if(match){
      if(!seen.has('weakest_assumption')){seen.add('weakest_assumption');parsed.weakest_assumption=normalizeTaskContractValue(match[1],200);}
      continue;
    }
    return null;
  }
...
function getMilestonePalette(index,total=0){
  const palette=[
    {accent:'#0e1a8a', tint:'rgba(14,26,138,.10)', border:'rgba(14,26,138,.22)', glow:'rgba(14,26,138,.14)'},
    {accent:'#2a36c8', tint:'rgba(42,54,200,.10)', border:'rgba(42,54,200,.22)', glow:'rgba(42,54,200,.14)'},
    {accent:'#4d67f2', tint:'rgba(77,103,242,.10)', border:'rgba(77,103,242,.22)', glow:'rgba(77,103,242,.14)'},
    {accent:'#6a8bff', tint:'rgba(106,139,255,.10)', border:'rgba(106,139,255,.22)', glow:'rgba(106,139,255,.14)'},
    {accent:'#8ba6ff', tint:'rgba(139,166,255,.10)', border:'rgba(139,166,255,.22)', glow:'rgba(139,166,255,.14)'},
    {accent:'#b5c6ff', tint:'rgba(181,198,255,.11)', border:'rgba(181,198,255,.22)', glow:'rgba(181,198,255,.14)'},
    {accent:'#d8e1ff', tint:'rgba(216,225,255,.12)', border:'rgba(42,54,200,.18)', glow:'rgba(42,54,200,.10)'}
  ];
...
function normalizeFounderTask(raw,options={}){
  ...
  const normalized={
    ...
    // New rich fields for Today tab and check-in system
    status:raw?.status||'pending',
    difficulty:raw?.difficulty||'medium',
    estimatedTimeMinutes:Number(raw?.time_minutes||raw?.estimatedTimeMinutes||0)||null,
    successCriteria:Array.isArray(raw?.success_criteria)?raw.success_criteria:(typeof raw?.success_criteria==='string'?raw.success_criteria.split('|').map(s=>s.trim()).filter(Boolean):[]),
    whatNotToDo:Array.isArray(raw?.what_not_to_do)?raw.what_not_to_do:(typeof raw?.what_not_to_do==='string'?raw.what_not_to_do.split('|').map(s=>s.trim()).filter(Boolean):[]),
    weakestAssumption:clipText(raw?.weakest_assumption||raw?.weakestAssumption||'',200)
  };
...
function ensureExecutionState(options={}){
  ...
  stages.forEach((stage,idx)=>{
    ...
    if(!Array.isArray(stage.taskIds)){
      ...
      const deduped=[...new Set(stage.taskIds.map(Number).filter(id=>Number.isFinite(id)&&id>0))];
      if(deduped.length!==stage.taskIds.length){stage.taskIds=deduped;changed=true;}
    }
...
function showApiKeyBanner(){
  if(!AI_CONFIGURED){
    ...
    banner.style.cssText='position:fixed;top:52px;left:12px;right:12px;z-index:200;background:rgba(253,251,246,.96);color:#0a0d18;padding:10px 16px;display:flex;align-items:center;gap:12px;font-family:JetBrains Mono,monospace;font-size:11px;font-weight:700;border:1px solid rgba(42,54,200,.22);border-radius:18px;box-shadow:0 18px 42px rgba(10,13,24,.12);';
    banner.innerHTML='AI KEY NOT SET — AI features will not work. <a href="https://platform.openai.com/api-keys" target="_blank" style="color:#2a36c8;text-decoration:underline;">Get key →</a> then set <code style="background:rgba(42,54,200,.08);color:#0e1a8a;padding:2px 7px;border-radius:999px;">OPENAI_API_KEY</code> in server environment variables. <button onclick="this.parentNode.remove()" style="margin-left:auto;background:#2a36c8;color:#fff;border:1px solid #2a36c8;border-radius:999px;padding:4px 10px;cursor:pointer;font-family:inherit;font-size:10px;font-weight:700;">Dismiss</button>';
...
const S = {
  user:{name:'',project:'',idea:'',goal:'',deadline:'',hours:'',blockers:'',styles:[],win:'',mode:'b2c',role:'user',stage:'',built:'',niche:'',audience:'',resources:'',constraints:'',
    // New fields
    projectType:'startup',experienceLevel:'intermediate',language:'en',todayTaskId:null,projects:[]
  },
...
async function flushCloudWriteQueue(){
  ...
  try{
    ...
    _cloudSyncFailCount=0;
  }catch(err){
    _cloudSyncFailCount++;
    logAppError('cloud_sync',err);
    if(CURRENT_AUTH_USER&&CURRENT_AUTH_USER.uid===uid){
      keys.forEach((key)=>CLOUD_WRITE_QUEUE.add(key));
    }
    if(_cloudSyncFailCount===3){
      toast2('Sync issue','Changes saved locally but cloud sync is failing. Check your connection.');
    }
  }finally{
...
function buildRoadmapOnboardingContext(){
  return {
    ...
    experience_level:S.user.experienceLevel||'intermediate',
    project_type:S.user.projectType||'startup',
    language:S.user.language||'en'
  };
}
...
function saveAll(){
  ...
  try{
    ...
    // Persist check-ins (keep last 200) and adaptation count
    if(Array.isArray(S.checkins))localStorage.setItem('sa_checkins',JSON.stringify(S.checkins.slice(-200)));
    localStorage.setItem('sa_adaptation_count',String(S.adaptationCount||0));
  }catch(e){
    if(e&&(e.name==='QuotaExceededError'||e.code===22||e.name==='NS_ERROR_DOM_QUOTA_REACHED')){
      toast2('Storage full','Browser storage is full — some data may not be saved. Clear site data or export your roadmap.');
    }
  }
}
function loadAll(){
  ...
  try{
    ...
    // Load check-in history and adaptation count
    const ci=localStorage.getItem('sa_checkins');if(ci)S.checkins=JSON.parse(ci);
    const ac=localStorage.getItem('sa_adaptation_count');if(ac)S.adaptationCount=Number(ac)||0;
  }catch(e){logAppError('load_state',e,'Stored state may be corrupt — some data reset to defaults');}
  // Ensure collections always exist as arrays
  if(!Array.isArray(S.checkins)) S.checkins=[];
  if(!Array.isArray(S.chatHistory)) S.chatHistory=[];
  // Clear session marker so a page refresh never shows a phantom "In Progress" session
  if(S.user) S.user.sessionStart=null;
  // Seed executionTaskSeq above any loaded task ID to prevent ID collisions
  if(S.execution&&typeof S.execution.tasksById==='object'){
    const loadedMaxId=Object.keys(S.execution.tasksById).reduce((max,k)=>{const n=Number(k);return n>max?n:max;},0);
    if(loadedMaxId>=executionTaskSeq) executionTaskSeq=loadedMaxId+1;
  }
...
function detectTaskOutputLanguage(...values){
  // Respect user's explicit language setting
  if(S.user&&S.user.language==='ru') return 'Russian';
  if(S.user&&S.user.language==='en') return 'English';
...
- Blocker: ${ctx.blocker||'none apparent'}
- Hours/day: ${ctx.daily_hours||'not specified'}
- Experience level: ${ctx.experience_level||'intermediate'}
- ${horizonLabel}
- Active horizon: ${windowDays} days
${strategyDesc?`- Pace: ${strategyDesc}`:''}
...
Language: ${(S.user&&S.user.language==='ru')?'Russian':'English'}`;
}
...
Experience level: ${context.experience_level||S.user?.experienceLevel||'intermediate'}
Project type: ${context.project_type||S.user?.projectType||'startup'}
...
WHAT_NOT_TO_DO | avoid this | and this
WEAKEST_ASSUMPTION | ...
TASK_DETAIL_END

Rules:
- title <= 8 words, description/why/deliverable/done <= 16 words
- difficulty and time must match the user's experience level: beginners get easier tasks (30-60min), advanced get harder (60-120min)
- success_criteria: 2 concrete measurable outcomes separated by |
- what_not_to_do: 2 anti-patterns separated by |
- weakest_assumption: one thing that could make this task irrelevant
- No generic tasks. Make it specific to this project.
...
function isAdmin(){return ADMIN_EMAILS.includes(CURRENT_AUTH_USER?.email||'');}
...
function initBetaDeadlineInput(){
  ...
  if(goalInput) goalInput.addEventListener('input',()=>{goalInput.style.borderColor='';});
}
...
function obPrefillStep(step){
  const setVal=(id,val)=>{const el=document.getElementById(id);if(el&&!el.value&&val)el.value=val;};
  if(step===0){
    setVal('ob-nm',S.user.name||'');
    setVal('ob-proj',S.user.project||'');
  }
  if(step===1){
    setVal('ob-goal',S.user.goal||'');
    setVal('ob-deadline',S.user.deadline||'');
    const hrs=document.getElementById('ob-hrs');if(hrs&&!hrs.value&&S.user.hours)hrs.value=S.user.hours;
    setVal('ob-block',S.user.blockers||'');
    setVal('ob-idea',S.user.idea||'');
  }
  if(step===2){
    const stageEl=document.getElementById('ob-stage');if(stageEl&&!stageEl.value&&S.user.stage)stageEl.value=S.user.stage;
    setVal('ob-niche',S.user.niche||'');
    setVal('ob-audience',S.user.audience||'');
    setVal('ob-resources',S.user.resources||S.user.executionStyle||'');
    setVal('ob-win',S.user.win||'');
  }
}
function obGo(step){
  const maxStep=3;
  const safeStep=Math.max(0,Math.min(maxStep,Number(step)||0));
  obPrefillStep(safeStep);
...
function obNext(from){
  if(from===0){
    ...
    // Save new fields: project type and experience level
    S.user.projectType=window._obProjectType||'startup';
    S.user.experienceLevel=document.getElementById('ob-exp-level')?.value||'intermediate';
    S.user.language=S.user.language||'en';
    trackEvent('onboarding_started');
  }
  if(from===1){
    const goalEl=document.getElementById('ob-goal');
    const goalVal=String(goalEl?.value||'').trim();
    if(!goalVal){
      if(goalEl){goalEl.style.borderColor='var(--red)';goalEl.focus();}
      return;
    }
    S.user.goal=goalVal;
    const deadlineInput=document.getElementById('ob-deadline');
...
async function obGenerateRoadmap(){
  ...
  try{
    ...
    S.user.lastGenerationContext=JSON.parse(JSON.stringify(roadmapCtx));
    ...
    roadmapReady=true;
    saveAll();
    setTimeout(()=>maybeShowUpgradePrompt('roadmap_generated'),1500);
    logInfo({
      area:'frontend',
      module:'frontend/script.js',
    ...
    setFlowOverlayMode('prompt',{
      ...
      copy:mapAIErrorMessage(e,'roadmap'),
      actionsHtml:'<button class="btn btn-primary btn-sm" onclick="obGenerateRoadmap()">Try Again</button>'
        +'<button class="btn btn-ghost btn-sm" style="margin-left:8px" onclick="hideFlowOverlay();obGo(1)">Edit Context</button>'
    });
  }finally{
    if(!isRoadmapPipelineActive(pipelineId)) return;
    ...
    if(!roadmapReady&&!showRetry) hideFlowOverlay();
  }
}
...
function obLaunch(opts={}){
  ...
  try{saveAll();}catch(e){}
  markUserRegistered();
  // Always land on Today tab — the AI execution coach home
  const targetPage='today';
  gp(targetPage);
  feedLine(`System initialized for ${S.user.name}.`);
  trackEvent('app_start');
  trackEvent('onboarding_completed');
  if(S.roadmap) trackEvent('roadmap_generated');
  showApiKeyBanner();
...
function gp(id){
  if(id==='chat') id='notes';
  if(id==='notes'&&!AI_CHAT_ENABLED){ renderChatPage(); }
  if(id==='analytics'&&!isAdmin()){toast2('Restricted','Analytics is available only to admins');id='today';}
  closeTaskDetail(true);
  const topbarMap={
    today:{eyebrow:'Execution Coach',title:'Today'},
    progress:{eyebrow:'Execution Coach',title:'Progress'},
  ...
  const topbar=topbarMap[id]||topbarMap.today;
...
function switchWorkTab(tab){
  ...
  if(actions){
    actions.innerHTML=`<button class="btn btn-primary btn-sm" onclick="gp('today')">Start working</button>`;
  }
  closeTaskDetail(true);
  renderTasks();
}
...
const _upgradeMessages={
  roadmap_generated:'Your AI roadmap is ready! Upgrade to Pro for adaptive rebuilds and the full 8-week plan.',
  task_done:'Great progress! Upgrade to Pro to unlock daily AI coaching and smart roadmap replanning.',
  ai_limit:'You\'ve reached the free AI limit. Upgrade to Pro for unlimited AI access.'
};
let _upgradePromptLastShown=0;
function showUpgradePrompt(trigger){
  const bar=document.getElementById('upgrade-prompt-bar');
  const txt=document.getElementById('upgrade-prompt-text');
  if(!bar||!txt) return;
  txt.textContent=_upgradeMessages[trigger]||_upgradeMessages.roadmap_generated;
  bar.style.display='flex';
}
function hideUpgradePrompt(){
  const bar=document.getElementById('upgrade-prompt-bar');
  if(bar) bar.style.display='none';
  _upgradePromptLastShown=Date.now();
}
function maybeShowUpgradePrompt(trigger){
  if(S.billing.plan!=='free') return;
  // Don't show again within 2 minutes of last dismissal
  if(Date.now()-_upgradePromptLastShown<120000) return;
...
function applyUserToUI(){
  ...
  const aiCalls=S.progress.aiCallCount||0;
  chip.textContent=plan==='free'?`FREE · ${aiCalls} AI`:plan.toUpperCase();
  chip.className='plan-chip '+plan;
  chip.title=plan==='free'?`Free plan · ${aiCalls} AI calls · Click to upgrade`:'Click to manage plan';
  if(S.user.goal){const gl=document.getElementById('dash-goal-line');if(gl)gl.textContent='🎯 '+S.user.goal;}
  const dstatSess=document.getElementById('dstat-sess');if(dstatSess)dstatSess.textContent=S.progress.sessions;
  const dstatStreak=document.getElementById('dstat-streak');if(dstatStreak)dstatStreak.textContent=S.progress.streak;
  const dstatPct=document.getElementById('dstat-pct');if(dstatPct)dstatPct.textContent=totalPct()+'%';
  updMilestoneBar();
  const analyticsTab=document.getElementById('tab-analytics');
  if(analyticsTab)analyticsTab.style.display=isAdmin()?'flex':'none';
  updateRoadmapRebuildUi();
  // Sync new settings fields
  const langEl=document.getElementById('set-lang');
  if(langEl)langEl.value=S.user.language||'en';
  const lvlEl=document.getElementById('set-exp-level');
  if(lvlEl)lvlEl.value=S.user.experienceLevel||'intermediate';
}
...
function buildRoadmapDetailPanel(index,opts={}){
  ...
  const stageTasksPreview=stageTasks.slice(0,7).map((task)=>{
    const why=clipText(task.whyItMatters||task.description||'',90);
    const diff=task.difficulty?({easy:'Easy',medium:'Medium',hard:'Hard'}[task.difficulty]||''):'';
    const mins=task.estimatedTimeMinutes;
    const timeLabel=mins?`${mins}min`:(task.estimateHours?`${task.estimateHours}h`:'');
    const isDone=Boolean(task.done||task.status==='done');
    const taskId=Number(task.id);
    return `<div class="rm-task-preview-item">
      <div class="rm-task-preview-title" style="${isDone?'text-decoration:line-through;opacity:.55':''}">
        ${isDone?'✓ ':''}${escHtml(getTaskTitle(task))}
      </div>
      ${why?`<div class="rm-task-preview-meta" style="margin:2px 0 4px;font-style:italic;">${escHtml(why)}</div>`:''}
      <div class="rm-task-preview-meta">
        <span>${escHtml(taskPrioLabel(task.prio||task.priority))}</span>
        ${diff?`<span>⊡ ${escHtml(diff)}</span>`:''}
        ${timeLabel?`<span>⏱ ${escHtml(timeLabel)}</span>`:''}
        <span>${escHtml(task.deadline||'No deadline')}</span>
      </div>
      ${!isDone?`<div style="display:flex;gap:4px;margin-top:4px;">
        <button class="btn btn-ghost btn-sm" onclick="openTaskDetail(${taskId})">Open</button>
        <button class="btn btn-ghost btn-sm" onclick="checkinFromDetail(${taskId},'too_hard')">↓ Easier</button>
        <button class="btn btn-ghost btn-sm" onclick="checkinFromDetail(${taskId},'not_relevant')">✕ Skip</button>
      </div>`:''}
    </div>`;
  }).join('');
  ...
  const detailActions=allowGenerationActions
    ...
    <button class="btn btn-ghost btn-sm" onclick="copyMilestoneSummary(${index})">⧉ Copy Summary</button>
  </div>`
    : `<div class="rm-detail-actions"><button class="btn btn-ghost btn-sm" onclick="copyMilestoneSummary(${index})">⧉ Copy Summary</button></div>`;
  return `<div class="rm-detail-head">
    <div>
      <div class="rm-detail-kicker">Milestone snapshot</div>
      <div class="rm-detail-title" style="display:flex;align-items:center;gap:8px;">${escHtml(stageTitle)}${stage?`<button class="edit-btn" onclick="editMilestoneTitle(${index})" title="Rename milestone">✎</button>`:''}</div>
  ...
  <div class="rm-detail-list">
    ...
    <div>
      ...
      <div class="rm-related-list">${stageTasksPreview||(stageStatus==='active'
        ?`<div class="rm-related-item rm-related-item--brief"><div class="rm-check-icon">·</div><div class="rm-related-copy"><strong>No tasks yet</strong><span>This milestone is active — generate tasks to start executing.</span></div></div><div style="margin-top:8px"><button class="btn btn-primary btn-sm" onclick="generateTasksForStage(${index}).then(()=>renderRM()).catch(()=>renderRM())">Generate Tasks →</button></div>`
        :`<div class="rm-related-item rm-related-item--brief"><div class="rm-check-icon">·</div><div class="rm-related-copy"><strong>Tasks pending</strong><span>Tasks will appear when this milestone becomes active.</span></div></div>`)}</div>
    </div>
  </div>
  ${detailActions}`;
}
          ...
          <svg class="rm-roadmap-path" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">
            <defs>
              <linearGradient id="rm-roadmap-line-gradient" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%" stop-color="rgba(42,54,200,0.16)"/>
                <stop offset="50%" stop-color="rgba(42,54,200,0.92)"/>
                <stop offset="100%" stop-color="rgba(106,139,255,0.22)"/>
              </linearGradient>
...
async function genVariants(){
  ...
  try{
    ...
    if(previousRoadmap){
      ...
      rb.innerHTML='<div class="rm-gen-state"><div style="font-size:36px">⚠</div><div class="rm-gen-title">Generation Failed</div><div class="rm-gen-sub">'+escHtml(mapAIErrorMessage(e,'roadmap'))+'</div><button class="btn btn-primary" style="margin-top:10px" onclick="genVariants()">Try Again →</button></div>';
    }
...
function buildChatContextSummary(){
  ...
  const todayTask=getCurrentActiveTask();
  const todayStatus=todayTask?(todayTask.status||(todayTask.done?'done':'pending')):'none';
  const recentBlockers=getCheckinHistory().filter(c=>c.type==='blocked'&&c.note).slice(-3).map(c=>clipText(c.note,60));
  let planStatus='on track';
  if(S.user.deadline){
    const daysLeft=Math.ceil((new Date(S.user.deadline)-new Date())/(1000*60*60*24));
    if(daysLeft<0) planStatus='past deadline';
    else if(stageProgress&&stageProgress.pct<30&&daysLeft<14) planStatus='behind';
    else if(stageProgress&&stageProgress.pct>70) planStatus='ahead';
  }
  return [
    `Goal: ${normalizeChatText(ctx.primary_goal||S.user.goal,'not set')}`,
    `Deadline: ${S.user.deadline||'not set'} | Plan: ${planStatus}`,
    `Milestone: ${stage?clipText(stage.title||'Active milestone',48):'none'}${stageProgress?` (${stageProgress.done}/${stageProgress.total} done, ${stageProgress.pct}%)`:''}`,
    `Today's task: ${todayTask?`${clipText(getTaskTitle(todayTask),60)} [${todayStatus}]`:'none assigned'}`,
    ...
    completedTasks.length?`Done recently: ${completedTasks.map((task)=>clipText(getTaskTitle(task),44)).join(' | ')}`:'Done recently: none',
    recentBlockers.length?`Recent blockers: ${recentBlockers.join(' | ')}`:'Recent blockers: none',
...
function buildChatSystemCtx(mode='daily'){
  ...
  return `StriveAI execution coach for ${modeLabel}. Be concise, factual, and specific. No motivational filler. Use only the provided app context. If data is missing, say so briefly. Return ONLY valid JSON (no markdown fences): {"text":"<response>","action":null} OR {"text":"<response>","action":{"type":"create_task","title":"<max 8 words>","reason":"<why this helps, max 15 words>","description":"<what to do exactly, max 20 words>"}}. Only set a non-null action when the user explicitly asks to add a task, make something easier, or create a checklist.`;
}
function buildChatPrompt(message='',mode='daily'){
  ...
  if(mode==='post_session'){
    return [
      ...
      `Give 2-3 concise insights: one bottleneck, one focus recommendation, one concrete next step.`,
  ...
  return [
    `Mode: daily check-in.`,
    `Give a direct, actionable answer. Max 3 short sentences or bullet points.`,
    userMessage?`User asked: ${userMessage}`:'',
    `Context:\n${context}`
  ].filter(Boolean).join('\n\n');
}
...
function renderChatMsgs(){
  ...
  const histOffset=S.chatHistory.length-messages.length;
  const stream=messages.length?messages.map((m,i)=>{
    const histIdx=histOffset+i;
    const actionHtml=(m.role==='ai'&&m.action&&!m.actionApplied&&!m.actionDismissed)
      ?`<div class="chat-action-confirm"><div class="chat-action-label">Suggested: <strong>${escHtml(m.action.title||'')}</strong></div>${m.action.reason?`<div class="chat-action-reason">${escHtml(m.action.reason)}</div>`:''}<div class="chat-action-btns"><button class="btn btn-primary btn-sm" onclick="applyChatSuggestedAction(${histIdx})">Apply</button><button class="btn btn-ghost btn-sm" onclick="dismissChatAction(${histIdx})">Dismiss</button></div></div>`:'';
    return `<div class="chat-msg ${m.role}"><div class="msg-av ${m.role}">${m.role==='ai'?'S':S.user.name.charAt(0).toUpperCase()}</div><div class="msg-bubble">${renderMarkdownishText(m.content)}${actionHtml}</div></div>`;
  }).join(''):emptyState;
...
function applyChatSuggestedAction(histIdx){
  const msg=S.chatHistory[histIdx];
  if(!msg||!msg.action||msg.actionApplied||msg.actionDismissed) return;
  const action=msg.action;
  if(!hasExecutionStateReady()) return;
  const activeIdx=getExecutionActiveStageIndex();
  const stage=getExecutionStage(activeIdx);
  if(!stage) return;
  const newTask={
    id:resolveUniqueExecutionTaskId(nextExecutionTaskId()),
    title:action.title||'AI suggested task',
    ...
    prio:'high',
    priority:'high',
    linkedStageIndex:activeIdx,
    linkedStage:activeIdx+1,
    stageObjective:stage.objective||'',
    deadline:'',
  ...
  stage.taskIds.unshift(newTask.id);
  S.execution.tasksById[String(newTask.id)]=newTask;
  S.user.todayTaskId=newTask.id;
  S.execution.updatedAt=nowIso();
  ...
  renderTodayPage();
  updRoadmapProgress();
  toast2('Task added','Switched to new task in Today.');
  trackEvent('chat_action_applied');
}

function dismissChatAction(histIdx){
  const msg=S.chatHistory[histIdx];
  if(!msg) return;
...
async function sendChatMsg(msg,options={}){
  ...
  try{
    ...
    try{
      ...
      parsedText=normalizeChatText(parsed.text||'','I need a bit more context to help with that.');
      parsedAction=(parsed.action&&parsed.action.type)?parsed.action:null;
    }catch(_){
      parsedText=normalizeChatText(reply,'I need a bit more context to help with that.');
    }
    S.chatHistory.push({role:'ai',content:parsedText,action:parsedAction});
    saveAll();
  }catch(error){
    if(requestId!==chatRequestSeq) return;
    ...
    host.innerHTML=`<div class="tasks-session-hero">
      ...
      <div class="tasks-session-hero-side">
        ...
        <div class="tasks-session-actions">
          <button class="btn btn-primary btn-sm" onclick="gp('today')">Start working</button>
        </div>
      </div>
    </div>`;
    return;
...
function buildTaskDetailMarkup(task){
  ...
  const taskStatus=task.status||(task.done?'done':'pending');
  const statusLabel={done:'✓ Done',pending:'○ Pending',in_progress:'▶ In Progress',blocked:'⊘ Blocked',skipped:'→ Skipped',not_relevant:'✕ Not Relevant'}[taskStatus]||taskStatus;
  ...
  const whyItMatters=escHtml(task.whyItMatters||task.why_it_matters||'This task moves the project toward a verifiable result in the current sprint.');
  const deliverable=escHtml(task.deliverable||'A concrete artifact or measurable result.');
  const doneDefinition=escHtml(task.doneDefinition||task.done_definition||'There is a verifiable result that can be shown.');
  ...
  const timeMin=task.estimatedTimeMinutes||task.estimated_time_minutes||null;
  const timeLabel=timeMin?`${timeMin} min`:'—';
  const successCriteria=Array.isArray(task.successCriteria)&&task.successCriteria.length?task.successCriteria:[];
  const whatNotToDo=Array.isArray(task.whatNotToDo)&&task.whatNotToDo.length?task.whatNotToDo:[];
  const weakestAssumption=task.weakestAssumption||'';
  const isFromToday=String(document.body.dataset.activeView||'')==='today';
  return `<div class="task-detail-shell task-detail-shell--calendar">
    <div class="task-detail-headline">
      <div>
        <div class="task-detail-kicker">${stageLabel} · ${stageTitle}</div>
        <div class="task-detail-title" id="task-detail-title">${escHtml(getTaskTitle(task))}</div>
      </div>
      <div style="display:flex;gap:6px;align-items:center;">
        <button class="edit-btn" onclick="openEditFieldModal(${Number(task.id)},'title')" title="Edit title">✎</button>
        <button class="task-detail-close" onclick="closeTaskDetail()" aria-label="Close task detail">✕</button>
      </div>
    </div>
    <div class="task-detail-meta">
      <span class="task-detail-chip task-status-chip status-${taskStatus}">${statusLabel}</span>
      <span class="task-detail-chip prio-${prio}">${escHtml(taskPrioLabel(prio))}</span>
      <span class="task-detail-chip diff-${difficulty}">⊡ ${diffLabel}</span>
      <span class="task-detail-chip">⏱ ${timeLabel}</span>
      <span class="task-detail-chip" style="cursor:pointer" onclick="openEditFieldModal(${Number(task.id)},'deadline')" title="Edit due date">${deadlineLabel} ✎</span>
    </div>
    ...
    <div class="task-detail-section">
      <div class="task-detail-label" style="display:flex;justify-content:space-between;align-items:center;">Success criteria <button class="edit-btn" onclick="openEditFieldModal(${Number(task.id)},'successCriteria')" title="Edit criteria">✎</button></div>
      ${successCriteria.length?`<ul class="task-detail-list">${successCriteria.map(c=>`<li>${escHtml(c)}</li>`).join('')}</ul>`:'<p style="color:var(--muted);font-style:italic;">None set — click ✎ to add criteria.</p>'}
    </div>
    <div class="task-detail-section">
    ...
    <div class="task-detail-section">
      <div class="task-detail-label">Done when</div>
      <p>${doneDefinition}</p>
    </div>
    ${whatNotToDo.length?`<div class="task-detail-section task-detail-section--warn">
      <div class="task-detail-label">⚠ What NOT to do</div>
      <ul class="task-detail-list">${whatNotToDo.map(c=>`<li>${escHtml(c)}</li>`).join('')}</ul>
    </div>`:''}
    ${weakestAssumption?`<div class="task-detail-section task-detail-section--assumption">
      <div class="task-detail-label">⟁ Weakest assumption</div>
      <p>${escHtml(weakestAssumption)}</p>
    </div>`:''}
    <div class="task-detail-section">
    ...
    <div class="task-detail-actions-row">
      ${taskStatus!=='done'?`<button class="btn btn-success btn-sm" onclick="checkinFromDetail(${Number(task.id)},'done')">✓ Mark as Done</button>`:''}
      ${taskStatus!=='done'?`<button class="btn btn-ghost btn-sm" onclick="checkinFromDetail(${Number(task.id)},'blocked')">⊘ Blocked</button>`:''}
      ${taskStatus!=='done'?`<button class="btn btn-ghost btn-sm" onclick="checkinFromDetail(${Number(task.id)},'too_hard')">↓ Too Hard</button>`:''}
      ${taskStatus!=='done'?`<button class="btn btn-ghost btn-sm" onclick="checkinFromDetail(${Number(task.id)},'not_relevant')">✕ Not Relevant</button>`:''}
      ${(taskStatus==='done'||taskStatus==='blocked'||taskStatus==='not_relevant'||taskStatus==='skipped')?`<button class="btn btn-ghost btn-sm" onclick="undoTaskState(${Number(task.id)})">↺ Undo</button>`:''}
      <button class="btn btn-outline btn-sm" onclick="setTodayTask(${Number(task.id)});gp('today')">Set as Today's Task</button>
      <button class="btn btn-ghost btn-sm" onclick="closeTaskDetail()">← Back</button>
    </div>
  </div>`;
}
...
function saveTasks(){
  ...
  try{
    ...
    if(e?.name==='QuotaExceededError'||e?.code===22||e?.name==='NS_ERROR_DOM_QUOTA_REACHED'){
      toast2('Storage full','Browser storage is full. Clear site data or export your roadmap.');
    } else {
      logAppError('save_tasks',e);
    }
  }
}
...
/* ══ BILLING ══ */
function renderBillingBtns(){
  const plan=S.billing.plan;
  const freeBtn=document.getElementById('free-btn');
  if(freeBtn) freeBtn.textContent=plan==='free'?'✓ Current Plan':'Downgrade';
  // Current plan display
  const planName=document.getElementById('bc-plan-name');
  const planDetail=document.getElementById('bc-detail');
  const planStatus=document.getElementById('bc-status');
  if(planName) planName.textContent=plan==='pro'?'Pro Plan':plan==='team'?'Team Plan':'Free Plan';
  if(planDetail) planDetail.textContent=plan==='free'?'Basic tracking · Limited AI features':plan==='pro'?'Full AI roadmap · Adaptive rebuilds · 3 projects':'Everything in Pro · Team collaboration · Unlimited projects';
  if(planStatus){planStatus.textContent='Active';planStatus.className='bc-status'+(plan==='free'?' free':' active');}
  // Usage counters (free plan only)
  const usageSection=document.getElementById('billing-usage');
  const usageStats=document.getElementById('bu-stats');
  if(usageSection&&usageStats){
    const aiCalls=S.progress.aiCallCount||0;
    const tasksDone=S.progress.tasksDone||0;
    const roadmapCount=roadmapRebuildCount||0;
    const checkins=getCheckinHistory().length;
    usageSection.style.display=plan==='free'?'block':'none';
    usageStats.innerHTML=[
      `<div class="bu-stat"><div class="bu-stat-val">${aiCalls}</div><div class="bu-stat-lbl">AI calls</div></div>`,
      `<div class="bu-stat"><div class="bu-stat-val">${tasksDone}</div><div class="bu-stat-lbl">Tasks done</div></div>`,
      `<div class="bu-stat"><div class="bu-stat-val">${roadmapCount}</div><div class="bu-stat-lbl">Roadmaps</div><div class="bu-stat-limit">Pro: unlimited</div></div>`,
      `<div class="bu-stat"><div class="bu-stat-val">${checkins}</div><div class="bu-stat-lbl">Check-ins</div></div>`
    ].join('');
  }
}
function initPayPal(planKey,containerId){if(PAYPAL_CLIENT_ID==='YOUR_PAYPAL_CLIENT_ID_HERE'){toast2('PayPal not configured','See billing section setup guide');return;}if(S.paypalLoaded){_renderPayPalBtn(planKey,containerId);return;}const script=document.createElement('script');script.src=`${PAYPAL_SDK_URL}?client-id=${PAYPAL_CLIENT_ID}&vault=true&intent=subscription`;script.onload=()=>{S.paypalLoaded=true;_renderPayPalBtn(planKey,containerId);};script.onerror=()=>{logAppError('paypal_load',new Error('PayPal SDK failed to load'));toast2('Payment error','Could not load PayPal. Check your internet connection and try again.');};document.head.appendChild(script);}
function _renderPayPalBtn(planKey,containerId){const planId=planKey==='pro'?PLAN_PRO:PLAN_TEAM;const el=document.getElementById(containerId);if(!el)return;el.innerHTML='<div id="pp-inner-'+containerId+'"></div>';try{paypal.Buttons({style:{shape:'rect',color:'blue',layout:'vertical',label:'subscribe'},createSubscription:(data,actions)=>actions.subscription.create({'plan_id':planId}),onApprove:(data)=>{S.billing.plan=planKey;S.billing.subscriptionId=data.subscriptionID;saveAll();applyUserToUI();renderBillingBtns();toast2('Upgraded! 🎉','');},onError:(err)=>{logAppError('paypal_checkout',err);toast2('Payment failed','PayPal encountered an error. Please try again or contact support.');}}).render('#pp-inner-'+containerId);}catch(e){logAppError('paypal_render',e);el.innerHTML='<div style="font-family:var(--mono);font-size:11px;color:var(--red);padding:8px">PayPal error: '+escHtml(String(e))+'</div>';}}
...
function renderAnalytics(){
  ...
  drawLineChart('chart-visits',labels,visitArr,'Visits','#2a36c8');
  drawLineChart('chart-users',labels,sessArr,'Completed Sessions','#6a8bff');
  ...
  if(isAdmin()){
    if(!_adminData&&!_adminLoading) loadAdminData();
    else if(_adminData) renderAdminSection(_adminData,_adminSearchQuery);
  }
}
...
function drawLineChart(id,labels,data,label,color){const canvas=document.getElementById(id);if(!canvas)return;const ctx=canvas.getContext('2d');const W=canvas.parentElement.clientWidth||400,H=180;canvas.width=W*devicePixelRatio;canvas.height=H*devicePixelRatio;canvas.style.width=W+'px';canvas.style.height=H+'px';ctx.scale(devicePixelRatio,devicePixelRatio);const pad={t:10,r:10,b:28,l:32};const cw=W-pad.l-pad.r,ch=H-pad.t-pad.b;const max=Math.max(...data,1);ctx.fillStyle='#fdfbf6';ctx.fillRect(0,0,W,H);ctx.strokeStyle='rgba(10,13,24,.10)';ctx.lineWidth=1;for(let i=0;i<=4;i++){const y=pad.t+(ch/4)*i;ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(pad.l+cw,y);ctx.stroke();}ctx.fillStyle='#5b5f75';ctx.font='9px JetBrains Mono,monospace';ctx.textAlign='right';for(let i=0;i<=4;i++){const y=pad.t+(ch/4)*i;ctx.fillText(Math.round(max-(max/4)*i),pad.l-3,y+4);}const step=Math.ceil(labels.length/8);ctx.textAlign='center';labels.forEach((l,i)=>{if(i%step===0||i===labels.length-1){ctx.fillText(l,pad.l+(i/(labels.length-1||1))*cw,H-6);}});if(!data.some(v=>v>0)){ctx.fillStyle='#5b5f75';ctx.textAlign='center';ctx.font='11px JetBrains Mono';ctx.fillText('No data yet — use the app first',W/2,H/2);return;}const grad=ctx.createLinearGradient(0,pad.t,0,pad.t+ch);grad.addColorStop(0,color+'33');grad.addColorStop(1,color+'05');ctx.fillStyle=grad;ctx.beginPath();data.forEach((v,i)=>{const x=pad.l+(i/(data.length-1||1))*cw;const y=pad.t+(1-v/max)*ch;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.lineTo(pad.l+cw,pad.t+ch);ctx.lineTo(pad.l,pad.t+ch);ctx.closePath();ctx.fill();ctx.strokeStyle=color;ctx.lineWidth=2;ctx.lineJoin='round';ctx.beginPath();data.forEach((v,i)=>{const x=pad.l+(i/(data.length-1||1))*cw;const y=pad.t+(1-v/max)*ch;i===0?ctx.moveTo(x,y):ctx.lineTo(x,y);});ctx.stroke();}
function drawDurationChart(id,labels){const canvas=document.getElementById(id);if(!canvas)return;const ctx=canvas.getContext('2d');const W=canvas.parentElement.clientWidth||300,H=160;canvas.width=W*devicePixelRatio;canvas.height=H*devicePixelRatio;canvas.style.width=W+'px';canvas.style.height=H+'px';ctx.scale(devicePixelRatio,devicePixelRatio);ctx.fillStyle='#fdfbf6';ctx.fillRect(0,0,W,H);const sessionLog=S.progress.sessionLog||[];const now=new Date();const n=Math.min(labels.length,14);const data=Array.from({length:n},(_,i)=>{const d=new Date(now);d.setDate(d.getDate()-(n-1-i));const log=sessionLog.find(e=>e.date===d.toDateString());return log?log.duration:0;});const pad={t:8,r:8,b:24,l:28};const cw=W-pad.l-pad.r,ch=H-pad.t-pad.b;const max=Math.max(...data,1);const bw=cw/n-2;data.forEach((v,i)=>{const x=pad.l+(cw/n)*i+1;const bh=(v/max)*ch;const y=pad.t+ch-bh;ctx.fillStyle='rgba(42,54,200,0.34)';ctx.fillRect(x,y,bw,bh);ctx.fillStyle='#2a36c8';ctx.fillRect(x,y,bw,2);});ctx.fillStyle='#5b5f75';ctx.font='9px JetBrains Mono,monospace';ctx.textAlign='center';ctx.fillText('Avg: '+(S.progress.sessions>0?Math.round(S.progress.hours*60/S.progress.sessions):0)+'m',W/2,H-6);if(!data.some(v=>v>0)){ctx.fillStyle='#5b5f75';ctx.textAlign='center';ctx.font='11px JetBrains Mono';ctx.fillText('No sessions yet',W/2,H/2);}}
function drawFeatureChart(id,counts){const canvas=document.getElementById(id);if(!canvas)return;const ctx=canvas.getContext('2d');const W=canvas.parentElement.clientWidth||300,H=160;canvas.width=W*devicePixelRatio;canvas.height=H*devicePixelRatio;canvas.style.width=W+'px';canvas.style.height=H+'px';ctx.scale(devicePixelRatio,devicePixelRatio);ctx.fillStyle='#fdfbf6';ctx.fillRect(0,0,W,H);const labels=['dashboard','work','notes','roadmap','settings','analytics'];const colors=['#0e1a8a','#2a36c8','#4d67f2','#6a8bff','#8ba6ff','#b5c6ff'];const data=labels.map(l=>counts['page_'+l]||0);const max=Math.max(...data,1);const pad={t:6,r:52,b:6,l:6};const cw=W-pad.l-pad.r,ch=H-pad.t-pad.b;const bh=(ch/labels.length)-3;labels.forEach((l,i)=>{const v=data[i];const bw=(v/max)*cw;const y=pad.t+(ch/labels.length)*i;ctx.fillStyle=colors[i]+'33';ctx.fillRect(pad.l,y,cw,bh);ctx.fillStyle=colors[i];ctx.fillRect(pad.l,y,bw,bh);ctx.fillStyle='#0a0d18';ctx.font='9px JetBrains Mono,monospace';ctx.textAlign='left';ctx.fillText(l.toUpperCase(),pad.l+cw+4,y+bh-1);ctx.textAlign='right';ctx.fillStyle=colors[i];ctx.fillText(v,pad.l+cw-3,y+bh-1);});}
function drawHourlyChart(id,hourly){const canvas=document.getElementById(id);if(!canvas)return;const ctx=canvas.getContext('2d');const W=canvas.parentElement.clientWidth||300,H=160;canvas.width=W*devicePixelRatio;canvas.height=H*devicePixelRatio;canvas.style.width=W+'px';canvas.style.height=H+'px';ctx.scale(devicePixelRatio,devicePixelRatio);ctx.fillStyle='#fdfbf6';ctx.fillRect(0,0,W,H);const pad={t:8,r:8,b:24,l:8};const cw=W-pad.l-pad.r,ch=H-pad.t-pad.b;const max=Math.max(...hourly,1);const bw=cw/24-1;hourly.forEach((v,i)=>{const x=pad.l+(cw/24)*i;const bh=(v/max)*ch;const isWork=i>=9&&i<=18;ctx.fillStyle=isWork?'rgba(42,54,200,0.62)':'rgba(42,54,200,0.22)';ctx.fillRect(x,pad.t+ch-bh,bw,bh);});ctx.fillStyle='#5b5f75';ctx.font='9px JetBrains Mono,monospace';ctx.textAlign='center';[0,6,12,18,23].forEach(h=>{ctx.fillText(h+'h',pad.l+(cw/24)*h+bw/2,H-6);});}
...
function exportRoadmapMarkdown(){
  if(!S.roadmap||!S.roadmap.length){toast2('No roadmap','Generate a roadmap first.');return;}
  const stages=getRoadmapViewStages();
  const project=S.user.project||'My Project';
  const goal=S.user.goal||'';
  const deadline=S.user.deadline||'';
  let md=`# ${project} — Execution Roadmap\n`;
  if(goal) md+=`\n**Goal:** ${goal}\n`;
  if(deadline) md+=`**Deadline:** ${deadline}\n`;
  md+=`\n---\n\n`;
  stages.forEach((stage,i)=>{
    const exec=getExecutionStage(i);
    const title=exec?.title||stage.title||`Stage ${i+1}`;
    const objective=getStageObjectiveText(exec||stage)||'';
    const outcome=getStageOutcomeText(exec||stage)||'';
    const pct=getRoadmapStagePct(i);
    const status=getRoadmapStageStatus(i);
    const target=exec?.targetDate||stage.target_date||getRoadmapTargetDate(i,stages.length);
    const tasks=getTasksForStage(i,{includeArchived:false});
    md+=`## Stage ${i+1}: ${title}\n`;
    md+=`**Status:** ${status} · **Progress:** ${pct}% · **Target:** ${target}\n\n`;
    if(objective) md+=`**Objective:** ${objective}\n\n`;
    if(outcome) md+=`**Outcome:** ${outcome}\n\n`;
    if(tasks.length){
      md+=`### Tasks\n\n`;
      tasks.forEach(t=>{
        const done=t.done||t.status==='done';
        const dl=t.deadline?` · due ${t.deadline}`:'';
        md+=`- [${done?'x':' '}] **${getTaskTitle(t)}**${dl}\n`;
        const why=t.whyItMatters||t.why_it_matters||'';
        if(why) md+=`  - *Why:* ${why}\n`;
        if(Array.isArray(t.successCriteria)&&t.successCriteria.length){
          t.successCriteria.forEach(c=>{md+=`  - ✓ ${c}\n`;});
        }
      });
      md+='\n';
    }
  ...
  const safeName=(project.replace(/[^a-z0-9]+/gi,'-')||'roadmap').toLowerCase();
  downloadText(md,`${safeName}-roadmap.md`,'text/markdown');
  toast2('Exported','Roadmap downloaded as Markdown.');
}
function exportTodayChecklist(){
  const todayTask=getCurrentActiveTask();
  const stageIndex=getExecutionActiveStageIndex();
  const stageName=getActiveStageName();
  const project=S.user.project||'My Project';
  const today=new Date().toLocaleDateString('en-GB',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
  let md=`# ${project} — Today's Tasks\n**${today}** · ${stageName}\n\n`;
  if(todayTask){
    const done=todayTask.done||todayTask.status==='done';
    md+=`## Primary Task\n\n- [${done?'x':' '}] **${getTaskTitle(todayTask)}**\n`;
    const why=todayTask.whyItMatters||todayTask.why_it_matters||'';
    if(why) md+=`  - *Why:* ${why}\n`;
    if(Array.isArray(todayTask.successCriteria)&&todayTask.successCriteria.length){
      md+=`  - Success criteria:\n`;
      todayTask.successCriteria.forEach(c=>{md+=`    - [ ] ${c}\n`;});
    }
    md+='\n';
  }
  const pending=getTasksForStage(stageIndex,{includeArchived:false})
    .filter(t=>t.status!=='done'&&!t.done&&t.status!=='archived'&&(!todayTask||Number(t.id)!==Number(todayTask.id)))
    .slice(0,5);
  if(pending.length){
    md+=`## Up Next\n\n`;
    pending.forEach(t=>{md+=`- [ ] ${getTaskTitle(t)}\n`;});
    md+='\n';
  }
  if(!todayTask&&!pending.length){md+=`*No active tasks in current milestone.*\n`;}
  copyToClipboard(md);
  toast2('Copied','Today\'s checklist copied to clipboard.');
}
function copyMilestoneSummary(index){
  const stage=getExecutionStage(index);
  const wk=S.roadmap?.[index];
  if(!stage&&!wk){toast2('No milestone','');return;}
  const title=stage?.title||wk?.title||`Stage ${index+1}`;
  const objective=getStageObjectiveText(stage||wk)||'';
  const outcome=getStageOutcomeText(stage||wk)||'';
  const pct=getRoadmapStagePct(index);
  const target=stage?.targetDate||wk?.target_date||getRoadmapTargetDate(index,S.roadmap?.length||1);
  const tasks=getTasksForStage(index,{includeArchived:false});
  let text=`## ${title}\nProgress: ${pct}% · Target: ${target}\n\n`;
  if(objective) text+=`**Objective:** ${objective}\n\n`;
  if(outcome) text+=`**Outcome:** ${outcome}\n\n`;
  if(tasks.length){
    text+=`**Tasks (${tasks.length}):**\n`;
    tasks.forEach(t=>{
      const done=t.done||t.status==='done';
      text+=`- [${done?'x':' '}] ${getTaskTitle(t)}\n`;
    });
  }
  copyToClipboard(text);
  toast2('Copied','Milestone summary copied to clipboard.');
}

/* ══ EDITING & RECOVERY ══ */
function openEditFieldModal(taskId,field){
  const task=getExecutionTaskById(taskId);
  if(!task) return;
  const overlay=document.getElementById('edit-field-overlay');
  const titleEl=document.getElementById('edit-field-modal-title');
  const body=document.getElementById('edit-field-modal-body');
  if(!overlay||!titleEl||!body) return;
  if(field==='title'){
    titleEl.textContent='Edit Task Title';
    body.innerHTML=`<div class="checkin-field"><label class="checkin-label">Title</label><input class="ob-in" id="edf-title" type="text" value="${escHtml(getTaskTitle(task))}" maxlength="160" onkeydown="if(event.key==='Enter')saveEditField(${Number(taskId)},'title')"/></div><div style="display:flex;gap:8px;margin-top:14px;justify-content:flex-end"><button class="btn btn-ghost btn-sm" onclick="closeEditFieldModal()">Cancel</button><button class="btn btn-primary btn-sm" onclick="saveEditField(${Number(taskId)},'title')">Save</button></div>`;
    setTimeout(()=>document.getElementById('edf-title')?.focus(),50);
  }else if(field==='deadline'){
    titleEl.textContent='Edit Due Date';
    body.innerHTML=`<div class="checkin-field"><label class="checkin-label">Due date</label><input class="ob-in" id="edf-deadline" type="date" value="${escHtml(task.deadline||'')}"/></div><div style="display:flex;gap:8px;margin-top:14px;justify-content:flex-end"><button class="btn btn-ghost btn-sm" onclick="closeEditFieldModal()">Cancel</button><button class="btn btn-primary btn-sm" onclick="saveEditField(${Number(taskId)},'deadline')">Save</button></div>`;
    setTimeout(()=>document.getElementById('edf-deadline')?.focus(),50);
  }else if(field==='successCriteria'){
    const cur=Array.isArray(task.successCriteria)?task.successCriteria.join('\n'):'';
    titleEl.textContent='Edit Success Criteria';
    body.innerHTML=`<div class="checkin-field"><label class="checkin-label">Success criteria (one per line)</label><textarea class="ob-in" id="edf-criteria" rows="5" style="resize:vertical">${escHtml(cur)}</textarea></div><div style="display:flex;gap:8px;margin-top:14px;justify-content:flex-end"><button class="btn btn-ghost btn-sm" onclick="closeEditFieldModal()">Cancel</button><button class="btn btn-primary btn-sm" onclick="saveEditField(${Number(taskId)},'successCriteria')">Save</button></div>`;
    setTimeout(()=>document.getElementById('edf-criteria')?.focus(),50);
  }
...
function saveEditField(taskId,field){
  const task=getExecutionTaskById(taskId);
  if(!task){closeEditFieldModal();return;}
  if(field==='title'){
    const val=(document.getElementById('edf-title')?.value||'').trim();
    if(!val){toast2('Empty title','Title cannot be empty.');return;}
    task.title=val;
  }else if(field==='deadline'){
    task.deadline=document.getElementById('edf-deadline')?.value||'';
  }else if(field==='successCriteria'){
    const raw=document.getElementById('edf-criteria')?.value||'';
    task.successCriteria=raw.split('\n').map(l=>l.trim()).filter(Boolean);
  }
  S.execution.updatedAt=nowIso();
  saveTasks();saveAll();
  closeEditFieldModal();
...
function undoTaskState(id){
  const task=getExecutionTaskById(id);
  if(!task) return;
  const prev=task.status||'done';
  task.status='pending';
  task.done=false;
  task.completedAt='';
  S.execution.updatedAt=nowIso();
  ...
  updRoadmapProgress();
  if(S.roadmap) renderRM();
  toast2('Undone',`Task moved back to Pending from ${prev}.`);
}
function editMilestoneTitle(index){
  const stage=getExecutionStage(index);
  if(!stage) return;
  const cur=stage.title||`Stage ${index+1}`;
  const val=prompt('Edit milestone title:',cur);
  if(val===null) return;
  const trimmed=val.trim();
  if(!trimmed||trimmed===cur) return;
  stage.title=trimmed;
  if(S.roadmap?.[index]) S.roadmap[index].title=trimmed;
  S.execution.updatedAt=nowIso();
  saveTasks();saveAll();
  renderRM();
  toast2('Milestone renamed',trimmed);
}

/* ══ ADMIN DASHBOARD ══ */
let _adminData=null,_adminLoading=false,_adminSearchQuery='';
async function loadAdminData(){
  if(!isAdmin()||!FB_DB) return;
  if(_adminLoading) return;
  _adminLoading=true;
  renderAdminSection(null,_adminSearchQuery);
  try{
    ...
    const users=await Promise.all(snap.docs.map(async(doc)=>{
      ...
      try{
        const [billingDoc,progressDoc,metaDoc,userDoc]=await Promise.all([
          FB_DB.collection('users').doc(doc.id).collection('kv').doc('sa_billing').get(),
          FB_DB.collection('users').doc(doc.id).collection('kv').doc('sa_progress').get(),
          FB_DB.collection('users').doc(doc.id).collection('kv').doc(SA_ROADMAP_META_KEY).get(),
          FB_DB.collection('users').doc(doc.id).collection('kv').doc('sa_user').get(),
        ]);
        if(billingDoc.exists){try{u.billingKv=JSON.parse(billingDoc.data().value||'{}');}catch(_){}}
        if(progressDoc.exists){try{u.progressKv=JSON.parse(progressDoc.data().value||'{}');}catch(_){}}
        u.hasRoadmap=metaDoc.exists;
        if(userDoc.exists){try{u.userKv=JSON.parse(userDoc.data().value||'{}');}catch(_){}}
      }catch(_){}
      return u;
    }));
    _adminData=users;
    renderAdminSection(users,_adminSearchQuery);
  }catch(e){
    logAppError('admin_load',e);
    renderAdminSection([],_adminSearchQuery,'Failed to load user data: '+(e.message||String(e)));
  }finally{
    _adminLoading=false;
  }
}
function adminSearch(q){
  _adminSearchQuery=q;
  if(_adminData) renderAdminSection(_adminData,q);
}
function renderAdminSection(users,searchQuery,errMsg){
  const el=document.getElementById('admin-section');
  if(!el) return;
  el.style.display='';
  if(users===null){
    el.innerHTML='<div class="admin-loading">Loading user data from Firestore…</div>';
    return;
  }
  if(errMsg){
    el.innerHTML=`<div class="admin-error">${escHtml(errMsg)}</div>`;
    return;
  }
  const q=(searchQuery||'').toLowerCase().trim();
  const filtered=q?users.filter(u=>(u.email||'').toLowerCase().includes(q)||(u.name||'').toLowerCase().includes(q)||(u.userKv?.project||'').toLowerCase().includes(q)):users;
  const now=Date.now();
  const day7=now-7*86400000;
  const activeUsers=users.filter(u=>{const ls=u.lastSeenAt?.toDate?.()?.getTime?.();return ls&&ls>day7;}).length;
  const roadmapsGenerated=users.filter(u=>u.hasRoadmap).length;
  const totalSessions=users.reduce((s,u)=>s+(Number(u.progressKv?.completedSessions)||0),0);
  const rowsHtml=filtered.length?filtered.map(u=>{
    const plan=u.billingKv?.plan||'free';
    const sessions=Number(u.progressKv?.completedSessions)||0;
    let lastSeen='—';
    if(u.lastSeenAt?.toDate) lastSeen=u.lastSeenAt.toDate().toLocaleDateString('en-GB');
    else if(u.lastSeenAt) lastSeen=new Date(u.lastSeenAt).toLocaleDateString('en-GB');
    const planCls=plan==='pro'?'admin-badge-pro':'';
    return `<tr><td class="admin-td">${escHtml(u.email||'—')}</td><td class="admin-td">${escHtml(u.name||'—')}</td><td class="admin-td"><span class="admin-badge ${planCls}">${escHtml(plan.toUpperCase())}</span></td><td class="admin-td">${u.hasRoadmap?'✓':'—'}</td><td class="admin-td">${sessions}</td><td class="admin-td">${lastSeen}</td></tr>`;
  }).join(''):`<tr><td colspan="6" class="admin-td-empty">No users match "${escHtml(q)}"</td></tr>`;
  el.innerHTML=`<div class="admin-wrap"><div class="admin-head">Admin Dashboard</div>
  <div class="admin-metrics">
    <div class="admin-metric"><div class="admin-metric-val">${users.length}</div><div class="admin-metric-lbl">Total Signups</div></div>
    <div class="admin-metric"><div class="admin-metric-val">${activeUsers}</div><div class="admin-metric-lbl">Active (7d)</div></div>
    <div class="admin-metric"><div class="admin-metric-val">${roadmapsGenerated}</div><div class="admin-metric-lbl">Roadmaps</div></div>
    <div class="admin-metric"><div class="admin-metric-val">${totalSessions}</div><div class="admin-metric-lbl">Sessions Done</div></div>
  </div>
  <div class="admin-search-row">
    <input class="ob-in admin-search-in" placeholder="Search by email, name, or project…" value="${escHtml(q)}" oninput="adminSearch(this.value)"/>
    <button class="btn btn-ghost btn-sm" onclick="loadAdminData()">↺ Refresh</button>
  </div>
  <div class="admin-table-wrap"><table class="admin-table">
    <thead><tr><th class="admin-th">Email</th><th class="admin-th">Name</th><th class="admin-th">Plan</th><th class="admin-th">Roadmap</th><th class="admin-th">Sessions</th><th class="admin-th">Last Active</th></tr></thead>
...
// Local Chrome extension/reminder integration preserved from this build
...
function getLastActivityDate(){
  try{
    ...
    // Find last session_started or session_review_completed event
    const activityEvents=events.filter(e=>['session_started','session_review_completed','app_start'].includes(e.event));
    if(!activityEvents.length) return null;
    const lastTs=Math.max(...activityEvents.map(e=>e.ts||0));
    return lastTs>0?new Date(lastTs):null;
  }catch(e){ return null; }
}

function checkActivityAndScheduleReminder(){
  try{
    ...
    // Push notification if browser notifications granted
    if(typeof Notification!=='undefined'&&Notification.permission==='granted'){
      if(daysSinceActivity>=2){
        new Notification('StriveAI – Come back!',{
          body:`You've been away for ${Math.floor(daysSinceActivity)} days. Your streak is at risk! Jump back in.`,
          icon:'/favicon.ico'
        });
      }else if(streak>0&&daysSinceActivity>=1){
        new Notification('StriveAI - Protect your streak',{
          body:`You have a ${streak}-day streak. Don't lose it — complete a session today!`,
          icon:'/favicon.ico'
        });
      }
    }
    // Also try to send email reminder via backend
    const userEmail=CURRENT_AUTH_USER?.email;
    if(userEmail&&daysSinceActivity>=2){
      sendActivityReminderEmail({
        email:userEmail,
        name:S.user?.name||'builder',
...
async function sendActivityReminderEmail(data){
  try{
    const resp=await fetch('/api/notify/reminder',{
      method:'POST',
      headers:{'Content-Type':'application/json'},
...
function getExtensionState(){
  ...
  return {
    ...
    goal:String(session?.goal||stage?.objective||S.user?.goal||''),
    elapsed:getSessionTimerLabel(session),
    countdown:buildSessionCountdownLabel(session),
    plannedDuration:session?.plannedDuration||0,
    progress:buildSessionProgressPct(session),
    streak:S.progress?.streak||0,
    taskCount:(session?.taskIds||[]).length,
    stageName:String(stage?.title||''),
    userName:String(S.user?.name||'')
  };
}
// Expose to extension via window
window.__striveAI={
  getState:getExtensionState,
  startSession:openSessionOverlay,
  endSession:()=>endSession('completed')
};
// Listen for messages from extension
window.addEventListener('message',(e)=>{
  if(!e.data||e.data.source!=='striveai-extension') return;
  if(e.data.type==='get_state'){
    e.source?.postMessage({source:'striveai-page',type:'state',payload:getExtensionState()},e.origin);
  }
  if(e.data.type==='start_session') openSessionOverlay();
  if(e.data.type==='end_session') endSession('completed');
});

requestNotificationPermission();
setTimeout(checkActivityAndScheduleReminder, 3000);
setInterval(checkActivityAndScheduleReminder, 60*60*1000);

/* ══════════════════════════════════════════════════
   TODAY TAB — AI EXECUTION COACH MAIN PAGE
══════════════════════════════════════════════════ */
...
function getCurrentActiveTask(){
  ...
  // 1. If there's a pinned today task, use it
  const pinnedId=S.user.todayTaskId||null;
  if(pinnedId){
    const t=getExecutionTaskById(pinnedId);
    if(t&&!t.done&&t.status!=='done'&&t.status!=='not_relevant') return t;
  }

  // 2. Find first in_progress task in active stage
  const stageToSearch=stages[activeIdx]||stages[0];
  if(!stageToSearch) return null;
  const taskIds=Array.isArray(stageToSearch.taskIds)?stageToSearch.taskIds:[];
  const allTasks=taskIds.map(id=>getExecutionTaskById(id)).filter(Boolean);

  const inProgress=allTasks.find(t=>t.status==='in_progress'&&!t.done);
  if(inProgress) return inProgress;

  // 3. First pending task
  const pending=allTasks.find(t=>(!t.status||t.status==='pending')&&!t.done&&t.status!=='done'&&t.status!=='not_relevant'&&t.status!=='skipped');
  if(pending) return pending;

  // 4. Any undone task across all stages
  for(const stage of stages){
    const ids=Array.isArray(stage.taskIds)?stage.taskIds:[];
    for(const id of ids){
      const t=getExecutionTaskById(id);
      if(t&&!t.done&&t.status!=='done'&&t.status!=='not_relevant'&&t.status!=='skipped') return t;
    }
  }
  return null;
}

function setTodayTask(id){
  S.user.todayTaskId=Number(id)||null;
  saveAll();
  renderTodayPage();
}

function getActiveStageName(){
  const idx=getExecutionActiveStageIndex();
  const stage=getExecutionStage(idx);
  return stage?.title||`Stage ${idx+1}`;
}

function getMilestoneProgress(){
  if(!hasExecutionStateReady()) return {done:0,total:0,pct:0};
  const idx=getExecutionActiveStageIndex();
  const stage=getExecutionStage(idx);
  if(!stage) return {done:0,total:0,pct:0};
  const ids=Array.isArray(stage.taskIds)?stage.taskIds:[];
  const tasks=ids.map(id=>getExecutionTaskById(id)).filter(Boolean);
  const total=tasks.length;
...
function getTodayCheckins(){
  const all=getCheckinHistory();
  const today=new Date().toDateString();
  return all.filter(c=>new Date(c.ts).toDateString()===today);
}

function hasDailyCheckinToday(){
  return getCheckinHistory().some(c=>c.type==='daily'&&new Date(c.ts).toDateString()===new Date().toDateString());
}
...
function getStreak(){
  ...
  const days=new Set(history.map(c=>new Date(c.ts).toDateString()));
  let streak=0;
...
function renderTodayPage(){
  const task=getCurrentActiveTask();

  // Context bar
  const ctxProject=document.getElementById('today-ctx-project');
  const ctxMilestone=document.getElementById('today-ctx-milestone');
  if(ctxProject) ctxProject.textContent=S.user.project||'Your Project';
  if(ctxMilestone) ctxMilestone.textContent=getActiveStageName();

  const emptyEl=document.getElementById('today-empty');
  const contentEl=document.getElementById('today-task-content');
  ...
  if(!task){
    ...
    const statusLabels={done:'✓ Done',pending:'○ Pending',in_progress:'▶ In Progress',blocked:'⊘ Blocked',skipped:'→ Skipped',not_relevant:'✕ Not Relevant'};

    setText('today-task-title', getTaskTitle(task));
    setText('today-task-milestone', getActiveStageName());
    setText('today-task-time', task.estimatedTimeMinutes?(task.estimatedTimeMinutes+' min'):'—');
    setText('today-task-difficulty', ({easy:'Easy ●',medium:'Medium ●●',hard:'Hard ●●●'}[task.difficulty]||'—'));
    setText('today-task-status-badge', statusLabels[status]||status);

    const statusBadge=document.getElementById('today-task-status-badge');
    if(statusBadge){
      statusBadge.className='today-task-badge today-task-badge--status today-status-'+status;
    }

    setText('today-why', task.whyItMatters||task.why_it_matters||'Moves the project toward a verifiable milestone.');

    const criteriaEl=document.getElementById('today-criteria');
    const criteriaSection=document.getElementById('today-criteria-section');
    const criteria=Array.isArray(task.successCriteria)&&task.successCriteria.length?task.successCriteria:[];
    if(criteriaEl&&criteriaSection){
      if(criteria.length){
        criteriaSection.style.display='block';
        criteriaEl.innerHTML='<ul class="today-criteria-list">'+criteria.map(c=>`<li>${escHtml(c)}</li>`).join('')+'</ul>';
      } else {
    ...
    if(actionsEl){
      if(status==='done'){
        actionsEl.innerHTML=`<div class="today-done-state">✓ Task completed! <button class="btn btn-primary btn-sm" onclick="advanceToNextTask()">Next Task →</button></div>`;
      } else if(status==='pending'){
        actionsEl.innerHTML=`
          <button class="btn btn-primary btn-today-primary" onclick="startWorkingOnTask(${Number(task.id)})">▶ Start Working</button>
          <button class="btn btn-ghost btn-today-secondary" onclick="openCheckinModal('not_relevant')">✕ Not Relevant</button>
          <button class="btn btn-outline btn-today-ai" onclick="openTodayAIChat()">✦ Ask AI</button>
        `;
      } else if(status==='in_progress'){
        const _ss=S.user.sessionStart?new Date(S.user.sessionStart):null;
        const _elapsed=_ss?Math.floor((Date.now()-_ss.getTime())/60000):null;
        const _elLabel=_elapsed!==null?(_elapsed<1?'< 1 min':_elapsed+' min'):'';
        actionsEl.innerHTML=`
          <div class="today-session-active">▶ Session active${_elLabel?' · '+_elLabel:''}</div>
          <button class="btn btn-success btn-today-primary" onclick="openCheckinModal('done')">✓ Mark as Done</button>
          <button class="btn btn-ghost btn-today-secondary" onclick="openCheckinModal('blocked')">⊘ I'm Blocked</button>
          <button class="btn btn-ghost btn-today-secondary" onclick="openCheckinModal('too_hard')">↓ Too Hard</button>
          <button class="btn btn-ghost btn-today-secondary" onclick="openCheckinModal('not_relevant')">✕ Not Relevant</button>
          <button class="btn btn-outline btn-today-ai" onclick="openTodayAIChat()">✦ Ask AI</button>
        `;
      } else {
    ...
    // Progress mini bar
    const prog=getMilestoneProgress();
    const miniEl=document.getElementById('today-progress-mini');
    if(miniEl) miniEl.innerHTML=`<div class="today-mini-bar"><div class="today-mini-fill" style="width:${prog.pct}%"></div></div><span>${prog.done}/${prog.total} done</span>`;
  }

  // Daily check-in prompt
  const dailyPromptEl=document.getElementById('today-daily-prompt');
  if(dailyPromptEl){
    if(hasDailyCheckinToday()){
      dailyPromptEl.innerHTML=`<div class="today-daily-done">✓ Daily check-in done today</div>`;
    } else {
      dailyPromptEl.innerHTML=`<div class="today-daily-banner"><div class="today-daily-copy">End-of-day review: What shipped, what blocked, what changed?</div><button class="btn btn-outline btn-sm" onclick="openDailyCheckinModal()">📋 Daily Check-in</button></div>`;
    }
  }

  // Stats row
  const todayCheckins=getTodayCheckins();
  const allCheckins=getCheckinHistory();
  const prog=getMilestoneProgress();
  setText('tstat-done', todayCheckins.filter(c=>c.type==='done').length);
  setText('tstat-streak', getStreak());
  setText('tstat-pct', prog.pct+'%');
  setText('tstat-checkins', allCheckins.length);

  trackEvent('today_action_viewed');
}
...
function advanceToNextTask(){
  S.user.todayTaskId=null;
  S.user.sessionStart=null;
  saveAll();
  renderTodayPage();
}
...
function startWorkingOnTask(taskId){
  ...
  canonical.startedAt=nowIso();
  S.user.sessionStart=nowIso();
  S.user.todayTaskId=Number(taskId);
  S.execution.updatedAt=nowIso();
  saveAll();
  renderTodayPage();
  updRoadmapProgress();
...
/* ══════════════════════════════════════════════════
   CHECK-IN MODAL SYSTEM
══════════════════════════════════════════════════ */

let _checkinType=null;

function openCheckinModal(type){
  _checkinType=type;
  const overlay=document.getElementById('checkin-overlay');
  const titleEl=document.getElementById('checkin-modal-title');
  const bodyEl=document.getElementById('checkin-modal-body');
  if(!overlay||!bodyEl) return;

  const titles={done:'✓ Mark as Done',blocked:'⊘ What blocked you?',too_hard:'↓ Too Hard',not_relevant:'✕ Not Relevant'};
  if(titleEl) titleEl.textContent=titles[type]||'Check-in';

  if(type==='done'){
    bodyEl.innerHTML=`
      <div class="checkin-field">
        <label class="checkin-label">What did you complete?</label>
        <textarea class="ob-in checkin-textarea" id="ci-done-what" placeholder="Describe what you finished..." rows="3"></textarea>
      </div>
      <div class="checkin-field">
        <label class="checkin-label">Any result or evidence? <span style="opacity:.5">(optional)</span></label>
        <input class="ob-in" id="ci-done-evidence" placeholder="Link, screenshot, number, quote..."/>
      </div>
      <div id="ci-ai-response" class="ci-ai-response" style="display:none"></div>
      <div style="display:flex;gap:8px;margin-top:16px;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" onclick="closeCheckinModal()">Cancel</button>
        <button class="btn btn-success btn-sm" id="ci-submit-btn" onclick="submitCheckin('done')">Submit ✓</button>
      </div>`;
  } else if(type==='blocked'){
    bodyEl.innerHTML=`
      <div class="checkin-field">
        <label class="checkin-label">What blocked you?</label>
        <textarea class="ob-in checkin-textarea" id="ci-block-what" placeholder="Describe what stopped you..." rows="3"></textarea>
      </div>
      <div class="checkin-field">
        <label class="checkin-label">Blocker type</label>
        <select class="ob-in" id="ci-block-type">
          <option value="dont_know">I don't know how to do it</option>
          <option value="no_time">No time</option>
          <option value="too_hard">Too hard</option>
          <option value="not_relevant">Not relevant</option>
          <option value="need_person">Need another person</option>
      ...
      <div style="display:flex;gap:8px;margin-top:16px;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" onclick="closeCheckinModal()">Cancel</button>
        <button class="btn btn-primary btn-sm" id="ci-submit-btn" onclick="submitCheckin('blocked')">Submit & Get Help</button>
      </div>`;
  } else if(type==='too_hard'){
    bodyEl.innerHTML=`
      <div class="checkin-field">
        <label class="checkin-label">What makes it too hard?</label>
        <textarea class="ob-in checkin-textarea" id="ci-hard-what" placeholder="Too complex, missing skills, too big..." rows="3"></textarea>
      </div>
      <div class="ci-info-box">
        <div class="ci-info-icon">↓</div>
        <div>AI will break this task into a smaller, doable step you can do today.</div>
      </div>
      <div id="ci-ai-response" class="ci-ai-response" style="display:none"></div>
      <div style="display:flex;gap:8px;margin-top:16px;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" onclick="closeCheckinModal()">Cancel</button>
        <button class="btn btn-primary btn-sm" id="ci-submit-btn" onclick="submitCheckin('too_hard')">Break It Down</button>
      </div>`;
  } else if(type==='not_relevant'){
    bodyEl.innerHTML=`
      <div class="checkin-field">
        <label class="checkin-label">Why is this not relevant?</label>
        <textarea class="ob-in checkin-textarea" id="ci-nr-why" placeholder="Context changed, wrong priority, already done..." rows="3"></textarea>
      </div>
      <div class="ci-info-box">
        <div class="ci-info-icon">✦</div>
        <div>AI will replace this task with something more relevant to your current situation.</div>
      </div>
      <div id="ci-ai-response" class="ci-ai-response" style="display:none"></div>
      <div style="display:flex;gap:8px;margin-top:16px;justify-content:flex-end">
        <button class="btn btn-ghost btn-sm" onclick="closeCheckinModal()">Cancel</button>
        <button class="btn btn-primary btn-sm" id="ci-submit-btn" onclick="submitCheckin('not_relevant')">Replace Task</button>
      </div>`;
  }
...
async function submitCheckin(type){
  ...
  if(submitBtn){submitBtn.disabled=true;submitBtn.textContent='AI is thinking...';}

  const task=getCurrentActiveTask();
  if(!task){closeCheckinModal();return;}

  // Gather input
  let userNote='';
  let blockerType='';
  if(type==='done'){
    userNote=(document.getElementById('ci-done-what')?.value||'').trim();
    const evidence=(document.getElementById('ci-done-evidence')?.value||'').trim();
    if(evidence) userNote+=(userNote?' | ':'')+evidence;
  } else if(type==='blocked'){
    userNote=(document.getElementById('ci-block-what')?.value||'').trim();
    blockerType=document.getElementById('ci-block-type')?.value||'other';
  } else if(type==='too_hard'){
    userNote=(document.getElementById('ci-hard-what')?.value||'').trim();
  } else if(type==='not_relevant'){
    userNote=(document.getElementById('ci-nr-why')?.value||'').trim();
  }

  // Update task status
  const canonical=getExecutionTaskById(task.id);
  if(canonical){
    const statusMap={done:'done',blocked:'blocked',too_hard:'pending',not_relevant:'not_relevant'};
    canonical.status=statusMap[type]||type;
    canonical.done=type==='done';
    if(type==='done') canonical.completedAt=nowIso();
    canonical.checkinNote=userNote;
    canonical.checkinAt=nowIso();
    S.execution.updatedAt=nowIso();
  }
  ...
  // Activity tracking
  const today=new Date().toDateString();
  if(!S.progress.activityLog.includes(today)) S.progress.activityLog.push(today);
  updStreak();
  if(type==='done') S.progress.tasksDone=(S.progress.tasksDone||0)+1;

  // AI response
  let aiResult=null;
  try{
    aiResult=await getCheckinAIResponse(type,task,userNote,blockerType);
  }catch(e){console.warn('AI check-in response failed',e);}

  if(aiResult){
    checkin.aiSummary=aiResult.summary||'';
    checkin.roadmapChanged=!!aiResult.roadmap_changed;
    checkin.roadmapChangeReason=aiResult.roadmap_change_reason||'';
    if(aiResult.roadmap_changed) S.adaptationCount=(S.adaptationCount||0)+1;

    // Apply next action if AI returned one
    if(aiResult.next_action&&type!=='done'){
      applyAINextAction(aiResult.next_action, task, type);
    }
  }
  ...
  const aiBox=document.getElementById('ci-ai-response');
  if(aiBox&&aiResult){
    aiBox.style.display='block';
    aiBox.innerHTML=`
      <div class="ci-ai-summary">${escHtml(aiResult.summary||'Check-in recorded.')}</div>
      ${aiResult.roadmap_changed?`<div class="ci-ai-change">🔄 Roadmap updated: ${escHtml(aiResult.roadmap_change_reason||'')}</div>`:''}
      ${aiResult.next_action?`<div class="ci-ai-next"><strong>Next action:</strong> ${escHtml(aiResult.next_action.title||'')}</div>`:''}
    `;
    if(submitBtn){submitBtn.textContent='Close';submitBtn.disabled=false;submitBtn.onclick=()=>{closeCheckinModal();gp('today');};}
  } else {
    closeCheckinModal();
  }

  // Clear pinned task so next pending is picked; clear session on any terminal state
  if(type==='done'||type==='not_relevant'){S.user.todayTaskId=null;}
  if(type==='done'||type==='not_relevant'||type==='blocked'||type==='too_hard'){S.user.sessionStart=null;}
  // Upgrade prompt after value moments
  if(type==='done') setTimeout(()=>maybeShowUpgradePrompt('task_done'),800);
  ...
  // Track events
  const evMap={done:'task_marked_done',blocked:'task_marked_blocked',not_relevant:'task_marked_not_relevant',too_hard:'task_marked_too_hard'};
  trackEvent(evMap[type]||'checkin_submitted');
  trackEvent('checkin_submitted');
  trackEvent('next_action_generated');
}

async function getCheckinAIResponse(type,task,userNote,blockerType){
  const taskTitle=getTaskTitle(task);
  const proj=S.user.project||'the project';
  const lang=S.user.language==='ru'?'Russian':'English';
  const promptMap={
    done:`User completed task: "${taskTitle}"\nNote: ${userNote||'no note'}\nProject: ${proj}`,
    blocked:`User is blocked on task: "${taskTitle}"\nBlocker: ${userNote||'unspecified'}\nBlocker type: ${blockerType}\nProject: ${proj}`,
    too_hard:`Task is too hard for user: "${taskTitle}"\nReason: ${userNote||'unspecified'}\nProject: ${proj}\nExperience level: ${S.user.experienceLevel||'intermediate'}`,
    not_relevant:`Task not relevant: "${taskTitle}"\nReason: ${userNote||'unspecified'}\nProject: ${proj}`
  };
  const typeInstructions={
    done:'Congratulate briefly. Suggest the logical next step.',
    blocked:'Acknowledge the blocker. Suggest a simpler alternative action that avoids the blocker. Be direct, not just supportive.',
    too_hard:'Break the task into ONE small concrete step doable in 30-60 min. Be specific to the project.',
    not_relevant:'Replace the task with a more relevant action. Be specific to the current project stage.'
  };

  const prompt=`You are an AI execution coach. Respond in ${lang}.

${promptMap[type]||''}

Instruction: ${typeInstructions[type]||'Acknowledge and suggest next step.'}

Return ONLY valid JSON, no markdown:
{
  "summary": "1-2 sentence acknowledgment and insight",
  "roadmap_changed": true or false,
  "roadmap_change_reason": "short reason if changed, else empty string",
  "next_action": {
    "title": "concrete next task title (max 8 words)",
    "description": "what to do exactly (max 20 words)",
    "why_it_matters": "why this matters now (max 15 words)",
    "success_criteria": ["criterion 1", "criterion 2"],
    "estimated_time_minutes": 45
  }
}`;

  const raw=await ai(prompt,null,500,{},'chat');
  const jsonStr=raw.replace(/```json|```/g,'').trim();
  return JSON.parse(jsonStr);
}

function applyAINextAction(nextAction,currentTask,checkinType){
  if(!nextAction||!nextAction.title) return;
  if(!hasExecutionStateReady()) return;
  ...
  // For too_hard: replace current task with a simpler sub-task
  // For blocked/not_relevant: insert new task at the top
  const newTask={
    id:resolveUniqueExecutionTaskId(nextExecutionTaskId()),
    title:nextAction.title,
    description:nextAction.description||'',
    whyItMatters:nextAction.why_it_matters||'',
    why_it_matters:nextAction.why_it_matters||'',
    successCriteria:Array.isArray(nextAction.success_criteria)?nextAction.success_criteria:[],
    estimatedTimeMinutes:nextAction.estimated_time_minutes||45,
    difficulty:'easy',
    status:'pending',
    done:false,
    prio:'high',
  ...
  if(!Array.isArray(stage.taskIds)) stage.taskIds=[];
  const currentTaskIdx=stage.taskIds.indexOf(Number(currentTask.id));
  if(currentTaskIdx>=0){
    stage.taskIds.splice(currentTaskIdx+1,0,newTask.id);
  } else {
    stage.taskIds.unshift(newTask.id);
  }
  S.execution.tasksById[String(newTask.id)]=newTask;
  S.user.todayTaskId=newTask.id;
  S.execution.updatedAt=nowIso();
  trackEvent('roadmap_updated');
}

function checkinFromDetail(taskId,type){
  // Set the task as current and open check-in modal
  S.user.todayTaskId=taskId;
  saveAll();
  closeTaskDetail();
  openCheckinModal(type);
}
...
function openDailyCheckinModal(){
  ...
  bodyEl.innerHTML=`
    <div class="checkin-field">
      <label class="checkin-label">What did you ship today?</label>
      <textarea class="ob-in checkin-textarea" id="ci-daily-shipped" placeholder="Completed tasks, progress made, deliverables..." rows="2"></textarea>
    </div>
    <div class="checkin-field">
      <label class="checkin-label">What blocked progress? <span style="opacity:.5">(optional)</span></label>
      <textarea class="ob-in checkin-textarea" id="ci-daily-blocked" placeholder="Anything that slowed you down..." rows="2"></textarea>
    </div>
    <div class="checkin-field">
      <label class="checkin-label">What changed or did you learn?</label>
      <textarea class="ob-in checkin-textarea" id="ci-daily-changed" placeholder="New info, pivots, insights..." rows="2"></textarea>
    </div>
    <div id="ci-ai-response" class="ci-ai-response" style="display:none"></div>
    <div style="display:flex;gap:8px;margin-top:16px;justify-content:flex-end">
      <button class="btn btn-ghost btn-sm" onclick="closeCheckinModal()">Cancel</button>
      <button class="btn btn-primary btn-sm" id="ci-submit-btn" onclick="submitDailyCheckin()">Submit Review</button>
    </div>`;
  overlay.classList.add('on');
  document.body.classList.add('checkin-open');
}
...
async function submitDailyCheckin(){
  const submitBtn=document.getElementById('ci-submit-btn');
  if(submitBtn){submitBtn.disabled=true;submitBtn.textContent='AI is thinking...';}
  const shipped=(document.getElementById('ci-daily-shipped')?.value||'').trim();
  const blocked=(document.getElementById('ci-daily-blocked')?.value||'').trim();
  const changed=(document.getElementById('ci-daily-changed')?.value||'').trim();
  const today=new Date().toDateString();
  if(!S.progress.activityLog.includes(today)) S.progress.activityLog.push(today);
  updStreak();
  if(!Array.isArray(S.checkins)) S.checkins=[];
  const checkin={id:Date.now(),ts:nowIso(),type:'daily',taskId:null,taskTitle:'Daily Review',note:shipped,blockerNote:blocked,learnedNote:changed,roadmapChanged:false,aiSummary:''};
  let aiResult=null;
  try{aiResult=await getDailyCheckinAIResponse(shipped,blocked,changed);}catch(e){console.warn('Daily check-in AI failed',e);}
  if(aiResult){
    checkin.aiSummary=aiResult.summary||'';
    checkin.roadmapChanged=!!aiResult.roadmap_changed;
    checkin.roadmapChangeReason=aiResult.change_reason||'';
    if(aiResult.roadmap_changed) S.adaptationCount=(S.adaptationCount||0)+1;
    if(Array.isArray(aiResult.new_tasks)&&aiResult.new_tasks.length&&hasExecutionStateReady()){
      const activeIdx=getExecutionActiveStageIndex();
      const stage=getExecutionStage(activeIdx);
      if(stage){
        if(!Array.isArray(stage.taskIds)) stage.taskIds=[];
        aiResult.new_tasks.slice(0,2).forEach(t=>{
          const newTask={
            id:resolveUniqueExecutionTaskId(nextExecutionTaskId()),title:t.title||'Follow-up task',description:t.description||t.reason||'',
            whyItMatters:t.reason||'',why_it_matters:t.reason||'',successCriteria:[],
            estimatedTimeMinutes:30,difficulty:'easy',status:'pending',done:false,
            prio:'high',priority:'high',linkedStageIndex:activeIdx,linkedStage:activeIdx+1,
            stageObjective:stage.objective||'',deadline:'',
            created:new Date().toLocaleDateString('en-GB',{day:'numeric',month:'short'}),isAIGenerated:true
          };
          stage.taskIds.unshift(newTask.id);
          S.execution.tasksById[String(newTask.id)]=newTask;
        });
  ...
  if(aiBox&&aiResult){
    aiBox.style.display='block';
    aiBox.innerHTML=`
      <div class="ci-ai-summary">${escHtml(aiResult.summary||'Daily review recorded.')}</div>
      ${aiResult.roadmap_changed?`<div class="ci-ai-change">🔄 Roadmap adapted: ${escHtml(aiResult.change_reason||'')}</div>`:''}
      ${Array.isArray(aiResult.new_tasks)&&aiResult.new_tasks.length?`<div class="ci-ai-next"><strong>Added to roadmap:</strong>${aiResult.new_tasks.map(t=>`<div>• ${escHtml(t.title||'')}</div>`).join('')}</div>`:''}
    `;
    if(submitBtn){submitBtn.textContent='Close';submitBtn.disabled=false;submitBtn.onclick=()=>closeCheckinModal();}
  } else {
    if(submitBtn){submitBtn.textContent='Done';submitBtn.disabled=false;submitBtn.onclick=()=>closeCheckinModal();}
  }
  trackEvent('daily_checkin_submitted');
}

async function getDailyCheckinAIResponse(shipped,blocked,changed){
  const ctx=buildChatContextSummary();
  const lang=S.user.language||'English';
  const prompt=`You are an AI execution coach. Respond in ${lang}.

Daily review:
Shipped: ${shipped||'nothing reported'}
Blocked by: ${blocked||'nothing'}
Changed/learned: ${changed||'nothing'}

Project context:
${ctx}

Adapt the user's upcoming roadmap only if blockers or learnings require a concrete follow-up. Return ONLY valid JSON, no markdown:
{"summary":"1-2 sentence acknowledgment","roadmap_changed":true or false,"change_reason":"short reason or empty","new_tasks":[{"title":"max 8 words","description":"what to do, max 20 words","reason":"why now, max 15 words"}]}
Max 2 new_tasks. Use empty array if no new tasks needed.`;
  const raw=await ai(prompt,null,500,{},'chat');
  const jsonStr=raw.replace(/```json|```/g,'').trim();
  return JSON.parse(jsonStr);
}
...
function renderProgressPage(){
  ...
  const done=checkins.filter(c=>c.type==='done').length;
  const blocked=checkins.filter(c=>c.type==='blocked').length;
  const adaptations=S.adaptationCount||0;

  setText('pr-checkins', checkins.length);
  setText('pr-done-count', done);
  setText('pr-blocked-count', blocked);
  setText('pr-streak-count', getStreak());
  setText('pr-adaptations', adaptations);

  const listEl=document.getElementById('checkin-history-list');
  if(!listEl) return;

  if(!checkins.length){
    listEl.innerHTML='<div class="progress-empty">No check-ins yet. Complete your first task on the Today tab.</div>';
    return;
  }

  const sorted=[...checkins].reverse();
  const typeIcon={done:'✓',blocked:'⊘',too_hard:'↓',not_relevant:'✕',daily:'📋'};
  const typeLabel={done:'Done',blocked:'Blocked',too_hard:'Too Hard',not_relevant:'Not Relevant',daily:'Daily Review'};

  // Error log
  const errSection=document.getElementById('error-log-section');
  const errList=document.getElementById('error-log-list');
  if(errSection&&errList){
    const showDebug=_errorLog.length>0||new URLSearchParams(window.location.search).has('debug');
    errSection.style.display=showDebug?'block':'none';
    errList.innerHTML=_errorLog.length
      ?[..._errorLog].reverse().slice(0,20).map(e=>`<div class="chi-note" style="margin:4px 0"><span style="font-family:var(--mono);color:var(--muted);font-size:10px">[${escHtml(e.ts)}] ${escHtml(e.context)}</span> — ${escHtml(e.message)}${e.detail?` <span style="opacity:.6">(${escHtml(e.detail)})</span>`:''}</div>`).join('')
      :'<div class="progress-empty" style="font-size:11px">No errors recorded this session.</div>';
  }
  ...
  listEl.innerHTML=sorted.map(c=>{
    ...
    const isDailyType=c.type==='daily';
    const dailyDetails=isDailyType?[
      c.note?`<div class="chi-daily-field"><span class="chi-daily-label">Shipped:</span> ${escHtml(c.note)}</div>`:'',
      c.blockerNote?`<div class="chi-daily-field"><span class="chi-daily-label">Blocked:</span> ${escHtml(c.blockerNote)}</div>`:'',
      c.learnedNote?`<div class="chi-daily-field"><span class="chi-daily-label">Learned:</span> ${escHtml(c.learnedNote)}</div>`:''
    ].filter(Boolean).join(''):'';
    return `<div class="checkin-history-item checkin-type-${c.type}${isDailyType?' checkin-type-daily-review':''}">
      <div class="chi-header">
        <span class="chi-icon">${typeIcon[c.type]||'●'}</span>
        <span class="chi-type">${typeLabel[c.type]||c.type}</span>
        ${!isDailyType?`<span class="chi-task">${escHtml(c.taskTitle||'Task')}</span>`:''}
        <span class="chi-date">${dateStr} ${timeStr}</span>
      </div>
      ${dailyDetails}
      ${!isDailyType&&c.note?`<div class="chi-note">${escHtml(c.note)}</div>`:''}
      ${c.aiSummary?`<div class="chi-ai"><span class="chi-ai-tag">✦ AI</span> ${escHtml(c.aiSummary)}</div>`:''}
      ${c.roadmapChanged?`<div class="chi-change">🔄 Roadmap adapted: ${escHtml(c.roadmapChangeReason||'')}</div>`:''}
    </div>`;
  }).join('');
}
...
let _newProjType='startup';

function selectProjType(type){
  _newProjType=type;
  document.querySelectorAll('.proj-type-btn').forEach(b=>b.classList.remove('selected'));
  const btn=document.getElementById('ptype-'+type);
...
function showNewProjectModal(){
  ...
  if(overlay) overlay.classList.add('on');
  document.body.classList.add('checkin-open');
  _newProjType='startup';
  selectProjType('startup');
  // Default deadline to 30 days from now
  const d=new Date();d.setDate(d.getDate()+30);
  const dlInput=document.getElementById('new-proj-deadline');
  if(dlInput) dlInput.value=d.toISOString().slice(0,10);
  closeUserMenu();
  trackEvent('new_project_started');
}

function closeNewProjectModal(ev){
  if(ev&&ev.target!==document.getElementById('new-project-overlay')) return;
  const overlay=document.getElementById('new-project-overlay');
  if(overlay) overlay.classList.remove('on');
  document.body.classList.remove('checkin-open');
}
...
async function createNewProject(){
  ...
  const level=document.getElementById('new-proj-level')?.value||'intermediate';
  const hours=document.getElementById('new-proj-hours')?.value||'2–4 hours';
  ...
  // Save old project to history
  if(!Array.isArray(S.user.projects)) S.user.projects=[];
  if(S.user.project){
    S.user.projects.push({
      name:S.user.project,
      goal:S.user.goal||'',
      savedAt:nowIso(),
      execution:JSON.parse(JSON.stringify(S.execution||{})),
      roadmap:JSON.parse(JSON.stringify(S.roadmap||null)),
      checkins:JSON.parse(JSON.stringify(S.checkins||[]))
    });
  }
  ...
  S.user.project=name;
  S.user.goal=goal;
  S.user.deadline=deadline;
  S.user.experienceLevel=level;
  S.user.hours=hours;
  S.user.projectType=_newProjType;
  S.user.todayTaskId=null;
  S.roadmap=null;
  S.execution={stages:[],tasksById:{},id:'exec-'+Date.now(),updatedAt:nowIso()};
  S.checkins=[];
  S.adaptationCount=0;
  ...
  // Go to roadmap to generate new plan
  gp('roadmap');
  toast2('Project created','Generate your roadmap to get started.');
}
...
function showProjectSwitcher(){
  const overlay=document.getElementById('project-switcher-overlay');
  if(overlay) overlay.classList.add('on');
  document.body.classList.add('checkin-open');
  renderProjectSwitcherBody();
  closeUserMenu();
}

function closeProjectSwitcher(ev){
  if(ev&&ev.target!==document.getElementById('project-switcher-overlay')) return;
  const overlay=document.getElementById('project-switcher-overlay');
  if(overlay) overlay.classList.remove('on');
  document.body.classList.remove('checkin-open');
}

function renderProjectSwitcherBody(){
  const bodyEl=document.getElementById('project-switcher-body');
  if(!bodyEl) return;
  const projects=Array.isArray(S.user.projects)?S.user.projects:[];

  if(!projects.length){
    bodyEl.innerHTML=`<div style="font-family:var(--mono);font-size:12px;color:var(--muted)">
      No other projects yet. Create a new project to switch between them.
    </div>
    <div style="margin-top:12px">
      <button class="btn btn-primary btn-sm" onclick="closeProjectSwitcher();showNewProjectModal()">＋ New Project</button>
    </div>`;
    return;
  }

  const current=`<div class="proj-switcher-current">
    <div class="proj-switcher-label">Current Project</div>
    <div class="proj-switcher-name">${escHtml(S.user.project||'Unnamed')}</div>
    <div class="proj-switcher-meta">${escHtml(S.user.goal||'')}</div>
  </div>`;

  const list=projects.map((p,i)=>`
    <button class="proj-switcher-item" onclick="switchToProject(${i})">
      <div class="proj-switcher-item-name">${escHtml(p.name||'Project '+i)}</div>
      <div class="proj-switcher-item-meta">${escHtml(p.goal||'')} · ${escHtml(p.savedAt?new Date(p.savedAt).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}):'')}</div>
    </button>
  `).join('');

  bodyEl.innerHTML=current+`<div class="proj-switcher-list-label">Other Projects</div>`+list+
    `<div style="margin-top:16px"><button class="btn btn-primary btn-sm" onclick="closeProjectSwitcher();showNewProjectModal()">＋ New Project</button></div>`;
}
...
function switchToProject(idx){
  ...
  projects.splice(idx,1,current);
  S.user.projects=projects;

  // Restore selected project
  S.user.project=p.name;
  S.user.goal=p.goal||'';
  S.roadmap=p.roadmap||null;
  S.execution=p.execution||{stages:[],tasksById:{},id:'exec-'+Date.now(),updatedAt:nowIso()};
  S.checkins=p.checkins||[];
  S.user.todayTaskId=null;
  ...
  if(S.roadmap){updDashboard();renderRM();updRoadmapProgress();}
  gp('today');
  toast2('Switched!','Now working on: '+p.name);
}
...
/* ══════════════════════════════════════════════════
   ONBOARDING PROJECT TYPE SELECTION
══════════════════════════════════════════════════ */

function obSelectProjectType(type){
  window._obProjectType=type;
  document.querySelectorAll('#obs-0 .ob-variant').forEach(el=>{
    el.classList.remove('on');
  });
  const varEl=document.getElementById('var-type-'+type);
  if(varEl) varEl.classList.add('on');
}
...
function closeForgotPw(){
  ...
  if(overlay) overlay.classList.remove('on');
  document.body.classList.remove('checkin-open');
  const status=document.getElementById('forgot-pw-status');
  const err=document.getElementById('forgot-pw-error');
...
async function sendPasswordReset(){
  ...
  const statusEl=document.getElementById('forgot-pw-status');
  const errEl=document.getElementById('forgot-pw-error');
  const email=(emailEl?.value||'').trim();
  if(!email){if(emailEl){emailEl.style.borderColor='var(--red)';emailEl.focus();}return;}
  ...
  try{
    ...
    if(typeof firebase!=='undefined'&&firebase.auth){
      ...
      // Supabase or other auth
      const authModule=await import('./services/auth.js').catch(()=>null);
      if(authModule?.resetPassword){
        await authModule.resetPassword(email);
      }
    }
    if(statusEl){statusEl.textContent='✓ Reset email sent. Check your inbox.';statusEl.style.display='block';}
  } catch(e){
...
function setProjectLanguage(val){
  S.user.language=val;
  saveAll();
  toast2('Language updated','AI tasks will now be generated in '+(val==='ru'?'Russian':'English')+'.');
}

function setExperienceLevel(val){
  S.user.experienceLevel=val;
  saveAll();
  toast2('Level updated','AI will calibrate task difficulty to: '+val+'.');
}

/* ══════════════════════════════════════════════════
   TODAY AI CHAT SHORTCUT
══════════════════════════════════════════════════ */

function openTodayAIChat(){
  const task=getCurrentActiveTask();
  gp('notes');
  if(task){
    const msg='Help me with this task: '+getTaskTitle(task);
    setTimeout(()=>sendChatMsg(msg,{mode:'daily'}),300);
  }
  trackEvent('task_ask_ai');
}

/* ══════════════════════════════════════════════════
   ANALYTICS EVENTS — ENHANCED TRACKING
══════════════════════════════════════════════════ */

const _analyticsQueue=[];
function trackEvent(name,props={}){
  if(typeof sa_event==='function'){
    try{sa_event(name,props);}catch(e){}
  }
  if(typeof gtag==='function'){
    try{gtag('event',name,props);}catch(e){}
  }
  _analyticsQueue.push({name,props,ts:Date.now()});
  // Track return-next-day retention
  if(name==='app_start'||name==='today_action_viewed'){
    const lastVisit=Number(localStorage.getItem('sa_last_visit')||0);
    const now=Date.now();
    const oneDayMs=86400000;
    if(lastVisit&&(now-lastVisit)>=oneDayMs&&(now-lastVisit)<(2*oneDayMs)){
      if(typeof sa_event==='function') try{sa_event('user_returned_next_day',{});}catch(e){}
    }
    localStorage.setItem('sa_last_visit',String(now));
  }
}

/* ══════════════════════════════════════════════════
   SESSION PERSISTENCE FIX
   Ensure user stays logged in after page reload
══════════════════════════════════════════════════ */

(function patchAuthPersistence(){
  // Patch auth state listener to redirect to Today instead of dashboard
  const origAuthRestore=window.authRestoreSession||null;
  window.__todayRedirectOnLogin=true;
})();

/* ══════════════════════════════════════════════════
   ROADMAP TASK STATUS NORMALIZATION
══════════════════════════════════════════════════ */

function normalizeTaskStatus(task){
  if(!task) return 'pending';
  if(task.status&&['pending','in_progress','done','blocked','skipped','not_relevant'].includes(task.status)){
    return task.status;
  }
...
/* ══════════════════════════════════════════════════
   INIT — wire up Today page on app load
══════════════════════════════════════════════════ */

(function wireupTodayInit(){
  const origMarkRegistered=window.markUserRegistered;
  window.markUserRegistered=function(){
    if(origMarkRegistered) origMarkRegistered.apply(this,arguments);
  };

  // Ensure S.checkins exists in state model
  const origSaveAll=window.saveAll;
  window.saveAll=function(){
    if(!S.checkins) S.checkins=[];
    if(origSaveAll) origSaveAll.apply(this,arguments);
  };
})();
