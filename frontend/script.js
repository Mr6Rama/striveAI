...
function buildTaskDetailMarkup(task){
  ...
  return `<div class="task-detail-shell task-detail-shell--calendar">
    ...
    ${successCriteria.length?`<div class="task-detail-section">
      <div class="task-detail-label">Success criteria</div>
      <ul class="task-detail-list">${successCriteria.map(c=>`<li>${escHtml(c)}</li>`).join('')}</ul>
    </div>`:''}
    <div class="task-detail-section">
...
async function submitCheckin(type){
  ...
  // Clear pinned task so next pending is picked
...
Return ONLY valid JSON, no markdown:
{
  ...
  const raw=await aiText(prompt,null,500,'');
...
function renderProgressPage(){
  ...
  const typeIcon={done:'✓',blocked:'⊘',too_hard:'↓',not_relevant:'✕'};
  const typeLabel={done:'Done',blocked:'Blocked',too_hard:'Too Hard',not_relevant:'Not Relevant'};

  listEl.innerHTML=sorted.map(c=>{
    const d=new Date(c.ts);
    const dateStr=d.toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'});
    const timeStr=d.toLocaleTimeString('en-GB',{hour:'2-digit',minute:'2-digit'});
    return `<div class="checkin-history-item checkin-type-${c.type}">
      <div class="chi-header">
        <span class="chi-icon">${typeIcon[c.type]||'●'}</span>
        <span class="chi-type">${typeLabel[c.type]||c.type}</span>
        <span class="chi-task">${escHtml(c.taskTitle||'Task')}</span>
        <span class="chi-date">${dateStr} ${timeStr}</span>
      </div>
      ${c.note?`<div class="chi-note">${escHtml(c.note)}</div>`:''}
...
function openTodayAIChat(){
  ...
  if(task){
    ...
    setTimeout(()=>{
      ...
      if(chatInput){chatInput.value=msg;chatInput.focus();}
    },200);
  } else {
    gp('notes');
  }
